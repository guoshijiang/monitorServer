...
static int ULANE_OpenFile(int* pFp)
{
  ...
  strLen = tmpStrLen;
	...
	sprintf(fileName, "%s/log/%s_json.log", "/MonitorForC", timeFileName); 
...
static void ULANE_JsonLogCore(const char *file, int line, int level, int status, const char *fmt, va_list args)
{
  char            str[ULANE_MAX_STRING_LEN];
  int	            strLen = 0;
  char            tmpStr[64];
...
void Ulane_JsonWriteLog(const char *file, int line, int level, int status, const char *fmt, ...)
{
  va_list args;
  ...
  va_start(args, fmt);
  ULANE_JsonLogCore(file, line, level, status, fmt, args);
  va_end(args);
  return ;
}
