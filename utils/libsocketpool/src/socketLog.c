...
//生成日志文件的时间
static int ULANE_SockFileTime(char* fileTime)
{
...
//日志文件中的时间
static int ULANE_SockCurTime(char* strTime)
{
	struct tm*		  tmTime = NULL;
	size_t			    timeLen = 0;
	time_t			    tTime = 0;	
...
static int ULANE_SockOpenFile(int* pFp)
{
  ...
  tmpStrLen = ULANE_SockFileTime(timeFileName);
  tmpStrLen = sprintf(str, "[%s] ", timeFileName);
  strLen = tmpStrLen;
	
	memset(fileName, 0, sizeof(fileName));
	sprintf(fileName, "%s/log/%s_sock.log", "/MonitorForC", timeFileName); 
...
static void ULANE_SockLogCore(const char *file, int line, int level, int status, const char *fmt, va_list args)
{
  char            str[ULANE_MAX_STRING_LEN];
  int	            strLen = 0;
  char            tmpStr[64];
  ...
  //加入LOG时间
  tmpStrLen = ULANE_SockCurTime(tmpStr);
...
void Socket_Log(const char *file, int line, int level, int status, const char *fmt, ...)
{
  ...
  va_start(args, fmt);
  ULANE_SockLogCore(file, line, level, status, fmt, args);
  va_end(args);
  return ;
}
